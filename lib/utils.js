...
export const exists = async (...args) => new Promise((r) => r(existsSync(...args)));
...
export async function getDiscordPath(branch) {
    ...
    switch (platform) {
        case 'win32': {
            ...
            if (actualBranch)
                capitlizedBranch = `${actualBranch[0].toUpperCase()}${actualBranch.substring(1, actualBranch.length)}`;
        ...
        case 'linux': {
            ...
            if (!await exists(outDir))
                outDir = join(homedir(), '.var', 'app', 'com.discordapp.Discord', 'config', 'discord');
            break;
        }
        default: {
...
const appNumberReplace = (str) => parseFloat(str.replace(/app-/g, '').replace(/\D+/g, ''));
export async function getAppPath(discordPath) {
    // idk what to name
...
export function getCorePath(appPath) {
    ...
    if (platform === 'win32')
        ...
        desktopCoreDir, readdirSync(desktopCoreDir).find(f => f.includes('discord_desktop_core')));
