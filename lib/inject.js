 ...
 */
export default async function inject(desktopCorePath, injectString, writeFn, exitFn) {
    const coreFile = await readFile(join(desktopCorePath, 'index.js'), { encoding: 'utf-8' });
    if (coreFile.split('\n').length !== 1) {
        if (coreFile.includes('pomcord'))
            return void writeFn?.(desktopCorePath).then(() => void exitFn?.('reinjected'));
        else
            return void exitFn?.('already_injected');
    }
    try {
        void writeFn?.(desktopCorePath);
    }
    catch (err) {
        return void exitFn?.('error', err);
    }
    try {
        writeFile(join(desktopCorePath, 'index.js'), injectString);
    }
    catch (err) {
        return void exitFn?.('error', err);
    }
